...
(function() {
  ...
  // Helper to serialize transaction to base64 BCS bytes
  const serializeTransaction = async (transaction) => {
    ...
    if (transaction && typeof transaction.build === 'function') {
      try {
        ...
        console.warn('[Sui Agent Wallet] build() failed:', e);
      }
    }
  ...
  // Check if transaction is Wallet Standard v2 format (has toJSON but no build)
  const isJsonV2Transaction = (tx) => {
    return tx && typeof tx === 'object' && typeof tx.toJSON === 'function' && typeof tx.build !== 'function';
  };
  ...
  const suiSignTransactionFeature = {
    'sui:signTransaction': {
      ...
      signTransaction: async (input) => {
        ...
        const chain = input.chain || currentNetwork;
        
        // Wallet Standard v2: toJSON object, send as json-v2 to Bridge for build+sign
        if (isJsonV2Transaction(tx)) {
          const json = await tx.toJSON();
          const jsonStr = typeof json === 'string' ? json : JSON.stringify(json);
          const result = await sendToBackground('signTransaction', {
            transaction: jsonStr,
            transactionFormat: 'json-v2',
            chain
          });
          return { bytes: result.bytes, signature: result.signature };
        }
        
        // Standard path: already BCS bytes or buildable
        const txData = await serializeTransaction(tx);
        const result = await sendToBackground('signTransaction', {
          transaction: txData,
          chain
        });
  ...
  const suiSignAndExecuteTransactionFeature = {
    'sui:signAndExecuteTransaction': {
      ...
      signAndExecuteTransaction: async (input) => {
        const tx = input.transaction;
        const chain = input.chain || currentNetwork;
        const network = chain.split(':')[1] || 'testnet';
        ...
        if (isJsonV2Transaction(tx)) {
          const json = await tx.toJSON();
          const jsonStr = typeof json === 'string' ? json : JSON.stringify(json);
          signResult = await sendToBackground('signTransaction', {
            transaction: jsonStr,
            transactionFormat: 'json-v2',
            chain
          });
        ...
        const execResp = await fetch(rpcUrl, {
          ...
          body: JSON.stringify({
            ...
            params: [
              ...
              { showEffects: true, showEvents: true, ...(input.options || {}) },
              'WaitForLocalExecution'
            ]
          })
        });
        const execData = await execResp.json();
        if (execData.error) throw new Error(execData.error.message || JSON.stringify(execData.error));
